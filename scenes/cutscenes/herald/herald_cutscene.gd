...
func _on_balloon_line_changed(line: DialogueLine) -> void:
	for tag in line.tags:
		...
		if tag == "activate_shader":
			var shaders = get_tree().get_nodes_in_group("low_vision_shader")
			if shaders.size() > 0: shaders[0].toggle_effect(true)
		...
		if tag == "deactivate_shader":
			var shaders = get_tree().get_nodes_in_group("low_vision_shader")
			if shaders.size() > 0: shaders[0].toggle_effect(false)
		...
		if tag == "minigame_menu":
			...
			var menu_scene = preload("res://scenes/cutscenes/herald/ScrollMinigame.tscn")
...
func _on_minigame_verification(is_successful: bool) -> void:
	if is_successful:
		if is_instance_valid(balloon): balloon.start(current_resource, "win_reaction")
	else:
		if is_instance_valid(balloon): balloon.start(current_resource, "fail_reaction")
...
func _end_cutscene() -> void:
	...
	# Assicuriamoci di spegnere lo shader giusto alla fine
	var shaders = get_tree().get_nodes_in_group("low_vision_shader")
	if shaders.size() > 0:
