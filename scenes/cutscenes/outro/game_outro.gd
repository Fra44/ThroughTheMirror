...
var dialogue_resource: DialogueResource = preload("res://dialogue/game_outro.dialogue")

const NEXT_SCENE_PATH := "res://scenes/reflection/ReflectionQuiz.tscn"
...
func _ready() -> void:
	...
	if is_instance_valid(balloon):
		...
		if is_instance_valid(balloon_fade_target):
			...
			push_warning("GameOutro: nessun CanvasItem trovato dentro Balloon. Fade balloon non possibile.")
...
func _change_portrait(path: String) -> void:
	if ResourceLoader.exists(path):
		...
		push_warning("GameOutro: immagine non trovata: " + path)
...
func _fade_out_balloon(duration: float = 0.45) -> void:
	if not is_instance_valid(balloon):
		return
...
func _fade_in(duration: float = 2.0) -> void:
...
func _end_outro() -> void:
	...
	if ResourceLoader.exists(NEXT_SCENE_PATH):
		...
		push_warning("GameOutro: scena successiva non trovata: " + NEXT_SCENE_PATH)
		queue_free()
